...
overlay_image = cv2.imread('dfdf.png')
...
start_time = None
last_detection_time = time.time()  # Initialize with the current time
stop_mp3 = True
    ...
    if len(indices) > 0:
        ...
        with open('detections.csv', 'a', newline='') as csvfile:
            ...
            for i in indices:
                ...
                # Overlay the image on the detection box
                overlay = cv2.resize(overlay_image, (w, h))
                frame[y:y+h, x:x+w] = cv2.addWeighted(frame[y:y+h, x:x+w], 1, overlay, 0.7, 0)
