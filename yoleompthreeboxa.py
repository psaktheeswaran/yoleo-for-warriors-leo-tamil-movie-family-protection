...
overlay_image = cv2.imread('dfdf.png', cv2.IMREAD_UNCHANGED)
    ...
    if len(indices) > 0:
        ...
        with open('detections.csv', 'a', newline='') as csvfile:
            ...
            for i in indices:
                ...
                # Overlay the PNG image on the detected object
                overlay = cv2.resize(overlay_image, (w, h))
                for c in range(0, 3):
                    frame[y:y + h, x:x + w, c] = frame[y:y + h, x:x + w, c] * (1 - overlay[:, :, 3] / 255.0) + overlay[:, :, c] * (overlay[:, :, 3] / 255.0)
